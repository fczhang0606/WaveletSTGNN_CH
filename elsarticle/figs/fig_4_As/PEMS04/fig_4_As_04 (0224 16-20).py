...
def As_heatmap(As, p, lp, n, ln) :
    ...
    plt.title (f'PEMS04 2018.02.{18+(p-218)//288} {(p-218)%288//12}:00', 
    ...
    plt.tight_layout()
    plt.savefig("fig_4_As_04 " + 
                f"02.{18+(p-218)//288} {(p-218)%288//12}:00" + 
                ".png", 
...
def As_heatmap_D(As, p1, lp1, p2, lp2, n, ln) :
    ...
    # 绘制热力图
    plt.figure(figsize=(20, 12), facecolor='white')         # 画布
    plt.title (f"PEMS04 2018.02.{18+(p1-218)//288} [{(p1-218)%288//12}:00-{(p2-218)%288//12}:00]", 
               fontsize=30, pad=20)                         # 标题
    plt.xlabel('Nodes-X', fontsize=30)                      # X
    plt.xticks(np.arange(0, ln, 1), np.arange(0, ln, 1))    # [0, 1, 2, ..., ln-1]
    plt.ylabel('Nodes-Y', fontsize=30)                      # Y
    ...
    plt.tight_layout()
    plt.savefig("fig_4_As_04 " + 
                f"02.{18+(p1-218)//288} [{(p1-218)%288//12}:00-{(p2-218)%288//12}:00]" + 
                ".png", 
                dpi=100, bbox_inches='tight')
...
if __name__ == "__main__" :
    ...
    # 载入文件
    file    = './PEMS04_As.npy' # 邻接矩阵
    As      = np.load(file)     # (3394, 307, 307)
    ...
    for i in range(1, 5) :  # 4小时段
        d   = 6             # 02.24
        h   = 16 + i        # 17:00, 18:00, 19:00, 20:00

        p1  = p0+d*ld+16*lh # 02.24-16:00
        lp1 = 3             # 
        p2  = p0+d*ld+h*lh  # 02.24-
        lp2 = 3             # 
        n   = 0             # nodes
        ln  = 60            # [n, n+ln-1]
